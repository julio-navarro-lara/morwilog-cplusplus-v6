...
detections_matrix = csvread('../det_detections.csv');

%plot(1:length(detections), detections);

%pause

false_positives_matrix = csvread('../det_false_positives.csv');
real_attacks_matrix = csvread('../det_real_attacks.csv');
...
real_attacks = real_attacks_matrix(2,:);

real_detections = detections-false_positives;

rate_fp = false_positives(2:end) ./ detections(2:end);

rate_real_det = real_detections(2:end) ./ real_attacks(2:end);
